...
if($request[0] == 'post') {
	...
	$post = $db->super_query("SELECT * FROM " . PREFIX . "_post p LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$field[0]} = '{$field[1]}' AND id = '{$row}'");
} elseif($request[0] == 'xf') {
	$field = explode('|', $request[1]);
	$post = $db->super_query("SELECT * FROM " . PREFIX . "_post p LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE xfields LIKE '%{$field[0]}|{$field[1]}%' AND id = '{$row}'");
}
if(count($post) > 0) {
	if( $config['allow_alt_url'] ) {
		if( $config['seo_type'] == 1 OR $config['seo_type'] == 2 ) {
			if( intval( $post['category'] ) and $config['seo_type'] == 2 ) {
				$full_link = $config['http_home_url'] . get_url( intval( $post['category'] ) ) . "/" . $row . "-" . $post['alt_name'] . ".html";
			} else {
				$full_link = $config['http_home_url'] . $row . "-" . $post['alt_name'] . ".html";
			}
		} else {
	...
	foreach ($tempFields[1] as $id => $value) {
		$temes = str_replace('%xf_'. $value .'%', $xfields[$value], $temes);
	}
