<?php
	...
	if(!is_array($_POST['data'])) parse_str($_POST['data'], $_data);
	else $_data = $_POST['data'];
...
HTML;
			...
			$turl = "https://api.telegram.org/bot". $_data['bot'] ."/sendMessage?chat_id=". $_data['chat'] ."&text=" . urlencode ( $message ) . "&parse_mode=HTML";
		...
		case 'settings':
			if (!mkdir($concurrentDirectory = ENGINE_DIR . '/inc/maharder/_config', 0777, true)
				...
				$mh_admin->generate_log('telegram', 'save_setting', serialize(sprintf('Папка "%s" не была создана',
																				$concurrentDirectory)));
			}
